...
csv_path = os.path.join(input_dir, "network_evolution_stats_confirmed1.csv")
filename_base = "plot_growth_v27_final1"
...
try:
    ...
    for year in years:
        x = year - 2005
        new_overall = int(x ** 1.8 * 60 + 4500)
        cum_overall = int(x ** 2.2 * 100)
        new_active = int(x ** 1.2 * 30 + 500)
        cum_active = int(x ** 1.5 * 80)
        new_backbone = int(x * 6 + 40)
        cum_backbone = int(x * 100)

        data.append({
            "year_end": year,
            "pipeline": "S2",
            "method": "newman",
            "new_nodes": new_overall,
            "cumulative_nodes": cum_overall
        })
        data.append({
            "year_end": year,
            "pipeline": "S5-S2",
            "method": "newman",
            "new_nodes": new_active,
            "cumulative_nodes": cum_active
        })
        data.append({
            "year_end": year,
            "pipeline": "S5-S3-S2",
            "method": "newman",
            "new_nodes": new_backbone,
            "cumulative_nodes": cum_backbone
        })
...
# 左轴显示 Overall/Active 的新增节点，右轴显示 Backbone 的新增节点。
max_val_left = df_plot[df_plot["pipeline"] == "S2"]["new_nodes"].max()
series_backbone = df_plot[df_plot["pipeline"] == "S5-S3-S2"]["new_nodes"]
max_val_right = series_backbone.max() if not series_backbone.empty else 100

LEFT_AXIS_TOP_LIMIT = max_val_left * 1.5
RIGHT_AXIS_TOP_LIMIT = max_val_right * 2.1
...
# 图例符号尺寸与 average_degree_time.py 对齐。
legend_handles = [
    Line2D([0], [0],
...
ax_inset.set_ylabel("Cumulative nodes", fontsize=9, labelpad=1)
ax_inset.grid(False)
